...
Tconv_named <- setNames(Tchg$Tconv, Tchg$iso)  # named vector for per-scenario country lookup
...
for (dt in 1:length(incs)) {  
  ...
  for (scen in sn) {
    ...
    common_iso <- Reduce(intersect, list(iso_g, iso_p, names(Tconv_named)))
    growthproj <- growthproj[iso_g %in% common_iso, ]
    popproj    <- popproj[match(common_iso, iso_p), ]
    ccd        <- (incs[dt] - 0.8) * Tconv_named[common_iso] / length(yrs)
